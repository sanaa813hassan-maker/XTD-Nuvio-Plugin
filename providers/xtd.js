 ...
 *  - Multiple qualities per item (360p → 1080p, when available)
 *  - Arabic + English .srt subtitles attached automatically
 *  - Arabic-dubbed versions (dubs) added as extra streams when available
...
// ─── Stream extraction ────────────────────────────────────────────────
async function extractStreams(subjectId, detailPath, se, ep, label) {
  ...
  for (var s = 0; s < streams.length; s++) {
    ...
    out.push({
      name: "XTD" + tag,
      title: "XTD " + quality + tag + " • " + formatBytes(st.size) + " • MP4",
      url: withRelay(st.url),
      quality: quality,
      size: formatBytes(st.size),
      language: label ? "ar" : undefined,
      provider: "xtd",
      headers: streamHeaders(),
      subtitles: subtitles
    });
...
// Arabic-dubbed version of the same subject (for cartoons/anime)
async function findArabicDub(subject) {
  try {
    var dubs = subject.dubs || [];
    for (var i = 0; i < dubs.length; i++) {
      var d = dubs[i];
      if (d.type === 0 && d.detailPath && d.subjectId) {
        var nm = String(d.lanName || d.lanCode || "").toLowerCase();
        if (nm.indexOf("ar") !== -1 || String(d.lanName || "").indexOf("عرب") !== -1) {
          return d;
        }
      }
    }
...
async function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  try {
    ...
    var results = await extractStreams(subject.subjectId, subject.detailPath, se, ep, null);

    // Arabic dub (anime/cartoons)
    var dub = await findArabicDub(subject);
    if (dub && (dub.subjectId !== subject.subjectId || dub.detailPath !== subject.detailPath)) {
      try {
        var dubStreams = await extractStreams(dub.subjectId, dub.detailPath, se, ep, "مدبلج");
        results = results.concat(dubStreams);
      } catch (e) { /* dub optional */ }
    }
